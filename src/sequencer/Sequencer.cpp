...
const uint8_t MIDI_BASE_NOTE = 36;
 ...
 *
 * Resets all steps, playhead, and running state to defaults. Validates the
 * integrity of the internal state array. If any issue is detected, sets an
 * internal error flag and returns false. Safe to call multiple times
 *
 ...
 */
void Sequencer::start() { state.running = true; }
 ...
 */
void Sequencer::reset() { resetState(); }
 ...
 * - Always send noteOff for the last note before sending noteOn for the new
 * note.
 * - If the new step is ON, send noteOn for the current note, set oscillator
 * frequency, and trigger the envelope.
 * - If the new step is OFF, send noteOff for the last note (if any) and release
 * the envelope.
 * - Handle repeated notes by sending noteOff then noteOn, even if the note is
 * the same.
...
void Sequencer::advanceStep(uint8_t current_uclock_step, int mm_distance,
                            bool is_button16_held, bool is_button17_held,
  ...
  // (monophonic)
  if (currentNote >= 0 && !currentStep.slide) {
    handleNoteOff();
        releaseEnvelope(); // Sets trigenv1 = true

  }

  // --- Auto-write distance sensor to step if no step is selected for edit and
  // gate is high ---
  if (selectedStepForEdit == -1 && currentStep.gate) {
    // Only record one type of data at a time, based on which record button is
    // held
    ...
    if (button17Held) {
      int mmVelocity = map(mm, 0, 1400, 300, 1000);   // Maps for velocity, doesn't let it go silent
      if (mmVelocity < 0)
        mmVelocity = 0;
      if (mmVelocity > 1000)
        mmVelocity = 1000;
  ...
  if (currentStep.gate) {
    ...
    // Trigger the envelope. This will cause re-articulation on every gated
    // step.
    ...
    freq1 = currentStep.filter *
            1.f; // Map filter 0.0-1.0 to 0-5000 Hz (adjust as needed)

    // Start the note with a fixed duration (e.g., 24 ticks for a 16th note at
    // 96 PPQN)
    startNote(new_midi_note, vel1, 24);

    lastNote =
        new_midi_note; // Update lastNote to the currently playing MIDI note.
  } else {
...
/**
 * @brief Instantly play a step for real-time feedback (does not advance
 * playhead).
 */
void Sequencer::playStepNow(uint8_t stepIdx) {
  ...
  // Clamp note index to scale size
  uint8_t scaleIndex = (currentStep.note >= scaleSize) ? 0 : currentStep.note;
  if (scaleIndex >= SCALE_ARRAY_SIZE)
    scaleIndex = 0;
  int new_midi_note = MIDI_BASE_NOTE + scale[0][scaleIndex];
  ...
  // (Assuming Velocity and FiltFreq are global variables used in audio
  // rendering)
  extern volatile uint8_t Velocity;
  extern volatile float FiltFreq;
  Velocity = static_cast<uint8_t>(currentStep.velocity * 127.0f);
  FiltFreq = currentStep.filter *
             5000.0f; // Map filter 0.0-1.0 to 0-5000 Hz (adjust as needed)
...
/**
 * @brief Convert absolute MIDI note to the offset scheme used by the audio
 * thread.
 *
 */
...
void Sequencer::setStepNote(uint8_t stepIdx, uint8_t noteIndex) {
  ...
  // Serial.println(state.steps[stepIdx].note);
}
...
void Sequencer::setStep(int index, bool gate, bool slide, int note,
  ...
  if (velocity < 0.0f || velocity > 1.0f) {
    // Serial.println("Sequencer::setStep: Velocity value out of range
    // (0.0f-1.0f).");
    return;
  }
  if (filter < 0.0f || filter > 1.0f) {
    // Serial.println("Sequencer::setStep: Filter value out of range
    // (0.0f-1.0f).");
    return;
  }
...
void Sequencer::setStep(int index, const Step &stepData) {
  ...
  if (stepData.note < 0 || stepData.note > 24) {
    // Serial.println("Sequencer::setStep: Note value in Step object out of
    // range (0-24).");
    return;
  }
  if (stepData.velocity < 0.0f || stepData.velocity > 1.0f) {
    // Serial.println("Sequencer::setStep: Velocity value in Step object out of
    // range (0.0f-1.0f).");
    return;
  }
  if (stepData.filter < 0.0f || stepData.filter > 1.0f) {
    // Serial.println("Sequencer::setStep: Filter value in Step object out of
    // range (0.0f-1.0f).");
    return;
  }
 ...
 */
uint8_t Sequencer::getPlayhead() const { return state.playhead; }

/**
 * @brief Check if the sequencer is currently running.
 * @return true if running, false otherwise.
 */
bool Sequencer::isRunning() const { return state.running; }
...
// This method is const-correct and does not allow modification of the internal
// state.
const SequencerState &Sequencer::getState() const { return state; }
...
/**
 * @brief Decrement the note duration counter. If zero, sends NoteOff and clears
 * state.
 */
